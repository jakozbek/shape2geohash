...
class GeohashStream extends Stream.Readable {
  constructor(shapeCoordinates, options) {
    ...
    this.geohashHeight = Math.abs(
      topLeftGeohashBbox[3] - topLeftGeohashBbox[1],
    );
  ...
  processNextRow() {
    ...
    if (this.options.hashMode === "envelope") {
      geohashes.push(
        ...this.processRowSegment(rowPolygon.geometry.coordinates),
      );
    } else {
      if (this.originalShape.geometry.type === "LineString") {
        ...
        let evenPairs;
        const firstPointOfFirstSegment =
          lineSegments[0].geometry.coordinates[0];
        ...
        lineSegments
          ...
          .forEach((lineSegment) => {
            const lineSegmentEnvelope =
              turfEnvelope(lineSegment).geometry.coordinates;
